...
public class BuildingBuilder {
     ...
     */
    public void AddRoom(String name, float surface, int lampWattage, float cubature, float heating){
        idx++;
        if(constructedComponent.GetChildCount() > 0){
            BuildingComposite Floor = (BuildingComposite) constructedComponent.GetLastChild();
            Floor.AddChild(new Room(idx, name, surface, lampWattage, cubature, heating));
        }
        else{
    ...
    public static BuildingComponent GetExamplaryBuilding() {
        ...
        factory.AddRoom("Room11", 10, 4, 10, 13);
        factory.AddRoom("Room12", 8, 6, 7, 13);
        factory.AddRoom("Room13", 8, 6, 7, 13);

        factory.AddFloor("Floor2");
        factory.AddRoom("Room21", 14, 6, 7, 13);
        factory.AddRoom("Room22", 14, 6,7, 13);

        factory.AddFloor("Floor3");
        factory.AddRoom("Room31", 14, 6, 7, 13);
        factory.AddRoom("Room32", 8, 6,7, 13);
        factory.AddRoom("Room33", 4, 6,7, 13);
